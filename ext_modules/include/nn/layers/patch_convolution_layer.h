#ifndef PATCH_CONVOLUTION_LAYER_H_
#define PATCH_CONVOLUTION_LAYER_H_
...
torch::Tensor PatchConvForward(torch::Tensor input, torch::Tensor weight,
  ...
  if (input.type().is_cuda()) {
    ...
    return cuda::PatchConvForward(input, weight, bias, kernel_h, kernel_w,
...
#endif
  {
    CHECK_CPU(weight);
    CHECK_CPU(bias);
    return cpu::PatchConvForward(input, weight, bias, kernel_h, kernel_w,
                                 stride_h, stride_w, pad_h, pad_w, dilation_h,
                                 dilation_w);
  }
}

std::vector<torch::Tensor> PatchConvBackward(
  ...
  if (grad_output.type().is_cuda()) {
    ...
    torch::Tensor grad_input = cuda::PatchConvBackwardInput(
        grad_output, weight, input.size(3), input.size(4), kernel_h, kernel_w,
        stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w);

    torch::Tensor grad_weight = cuda::PatchConvBackwardWeight(
...
#endif
  {
    ...
    torch::Tensor grad_input = cpu::PatchConvBackwardInput(
        grad_output, weight, input.size(3), input.size(4), kernel_h, kernel_w,
        stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w);

    torch::Tensor grad_weight = cpu::PatchConvBackwardWeight(
        grad_output, input, kernel_h, kernel_w, stride_h, stride_w, pad_h,
        pad_w, dilation_h, dilation_w);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("patch_conv_forward", &mapped_conv::nn::PatchConvForward,
        "Forward patch convolution");
  m.def("patch_conv_backward", &mapped_conv::nn::PatchConvBackward,
        "Backward patch convolution");
}
