#ifndef COMMON_PATCH_IM2COL_H_
#define COMMON_PATCH_IM2COL_H_
...
template <typename T>
__host__ __device__ void PatchIm2Col2D(
    const int64_t index, const T *data_im_ptr, const int64_t patches,
  ...
  // Location in the flattened image patches (i.e. which column in a row of the
  // column matrix)
  const int64_t x_col = index % width_col;

  // Number of output elements
  const int64_t N = height_out * width_out;

  // Locations in input
  const int64_t c_im   = index / width_col;
  const int64_t p_im   = x_col / N;
  const int64_t loc_im = x_col % N;
  const int64_t x_out  = loc_im % width_out;
  const int64_t y_out  = loc_im / width_out;
    ...
    for (int64_t x_kern = 0; x_kern < kernel_w; x_kern++) {
      ...
      data_col_ptr[index_col] =
          (x >= 0 && y >= 0 && x < width_im && y < height_im)
              ? data_im_ptr[c_im * patches * height_im * width_im +
                            p_im * height_im * width_im + y * width_im + x]
              : T(0);
...
template <typename T>
__host__ __device__ void PatchCol2Im2D(
    const int64_t index, const T *data_col_ptr, const int64_t patches,
  ...
  const int64_t p_im = (index / (width * height)) % patches;
  const int64_t c_im = index / (width * height * patches);
  ...
  for (int64_t y_out = y_out_start; y_out < y_out_end; y_out += 1) {
    for (int64_t x_out = x_out_start; x_out < x_out_end; x_out += 1) {
      // Get the location in the kernel
      int64_t y_k = (y_im - y_out * stride_h);
      int64_t x_k = (x_im - x_out * stride_w);
      ...
      if (y_k % dilation_h == 0 && x_k % dilation_w == 0) {
        ...
        int64_t data_col_index =
            c_im * kernel_h * kernel_w * patches * output_height *
                output_width +
            y_k * kernel_w * patches * output_height * output_width +
            x_k * patches * output_height * output_width +
            p_im * output_height * output_width + y_out * output_width + x_out;
        // std::cout << "data_col_index: " << data_col_index << std::endl;
