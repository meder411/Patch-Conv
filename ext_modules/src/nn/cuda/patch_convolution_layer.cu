...
torch::Tensor PatchConvForward(torch::Tensor input, torch::Tensor weight,
  ...
  torch::Tensor columns = torch::zeros(
      {kW * kH * nInputPlanes, nPatches * outputHeight * outputWidth},
      input.options());
  ...
  for (int b = 0; b < batchSize; b++) {
    // CUDA im2col
    PatchIm2Col2DLauncher(input[b], nInputPlanes, nPatches, inputHeight,
...
torch::Tensor PatchConvBackwardInput(torch::Tensor grad_output,
  ...
  for (int b = 0; b < batchSize; b++) {
    ...
    PatchCol2Im2DLauncher(columns, nInputPlanes, nPatches, inputHeight,
