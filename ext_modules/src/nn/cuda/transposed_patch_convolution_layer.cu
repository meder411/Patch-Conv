...
torch::Tensor TransposedPatchConvForward(torch::Tensor input,
  ...
  torch::Tensor columns = torch::zeros(
      {kW * kH * nOutputPlanes, nPatches * inputHeight * inputWidth},
      input.options());
  ...
  for (int b = 0; b < batchSize; b++) {
    ...
    PatchCol2Im2DLauncher(columns, nOutputPlanes, nPatches, outputHeight,
...
torch::Tensor TransposedPatchConvBackwardInput(torch::Tensor grad_output,
                                               torch::Tensor weight,
  ...
  const int64_t inputBatchStride =
      nInputPlanes * nPatches * inputHeight * inputWidth;
  for (int b = 0; b < batchSize; b++) {
    PatchIm2Col2DLauncher(grad_output[b], nOutputPlanes, nPatches,
...
torch::Tensor TransposedPatchConvBackwardWeight(torch::Tensor grad_output,
