...
torch::Tensor TransposedPatchConvForward(torch::Tensor input,
  ...
  const int64_t num_kernels =
      nOutputPlanes * nPatches * outputHeight * outputWidth;
  for (int b = 0; b < batchSize; b++) {
    ...
    if (input.dtype() == torch::kDouble) {
      PatchCol2Im2D<double>(num_kernels, columns, nPatches, outputHeight,
                            outputWidth, inputHeight, inputWidth, kernel_h,
                            kernel_w, pad_h, pad_w, stride_h, stride_w,
                            dilation_h, dilation_w, output[b]);
    } else if (input.dtype() == torch::kFloat) {
      PatchCol2Im2D<float>(num_kernels, columns, nPatches, outputHeight,
                           outputWidth, inputHeight, inputWidth, kernel_h,
                           kernel_w, pad_h, pad_w, stride_h, stride_w,
                           dilation_h, dilation_w, output[b]);
    }
...
torch::Tensor TransposedPatchConvBackwardInput(
  ...
  torch::Tensor columns = torch::zeros({kernel_w * kernel_h * nOutputPlanes,
                                        nPatches * inputHeight * inputWidth},
                                       grad_output.options());
  ...
  for (int b = 0; b < batchSize; b++) {
    if (grad_output.dtype() == torch::kDouble) {
      PatchIm2Col2D<double>(
          num_kernels, grad_output[b], nPatches, outputHeight, outputWidth,
    ...
    input_grad[b] =
        weight.view({weight.size(0), -1})
            .mm(columns)
            .view({nInputPlanes, nPatches, inputHeight, inputWidth});
  }
